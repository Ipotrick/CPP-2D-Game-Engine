	...
	*this = World();
	if (mapname_ == "standart")
	{
		...
		for (int i = 0; i < num; i++) {
			...
			c.add<Draw>(Draw(Vec4(rand()%1000 / 1000.0f, rand() % 1000 / 1000.0f, rand() % 1000 / 1000.0f, 1), Vec2(1, 1), 0.5f, Form::Rectangle));
			c.add<Tester>();
			spawn(ent);
		}
		...
		PhysicsBody trashSolidBody = PhysicsBody(0.5f, 0.5f, calcMomentOfIntertia(0.5, scale),0.9f);
