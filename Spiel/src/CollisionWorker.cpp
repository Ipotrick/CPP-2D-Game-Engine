...
void CollisionWorker::operator()()
{
	auto& world = poolData->world;
	auto& beginSensor = workerData->beginSensor;
	auto& endSensor = workerData->endSensor;
	auto& sensorCollidables = poolData->sensorCollidables;
	auto& beginDyn = workerData->beginDyn;
	auto& endDyn = workerData->endDyn;
	auto& dynCollidables = poolData->dynCollidables;
	auto& beginStat = workerData->beginStat;
	auto& endStat = workerData->endStat;
	auto& statCollidables = poolData->statCollidables;
	auto& collisionResponses = poolData->collisionResponses;
	auto& collisionInfos = workerData->collisionInfos;
...
void CollisionWorker::waitForUpdate() {
	std::unique_lock<std::mutex> switch_lock(syncData->mut);
	syncData->go.at(workerData->id) = false;
	syncData->cond.notify_all();
	syncData->cond.wait(switch_lock, [&]() { return syncData->go.at(workerData->id) == true; });	// wait for engine to give go sign
	if (syncData->run == false) run = false;
}

void CollisionWorker::waitForOtherWorkers()
{
...
//void CollisionWorker::updateStaticGrid()
//{
