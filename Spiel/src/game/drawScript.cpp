...
void drawScript(EntityHandle entity, const Transform& t, const Draw& d)
{
	if (Game::world.hasComp<TextureRef2>(entity)) {
		...
		EngineCore::renderer.submit(makeSprite(0, t.position, d.drawingPrio, d.scale, d.color, d.form, t.rotaVec, RenderSpace::WorldSpace, texRef.makeSmall()), LAYER_WORLD_MIDGROUND);
	}
	else {
		EngineCore::renderer.submit(makeSprite(0, t.position, d.drawingPrio, d.scale, d.color, d.form, t.rotaVec, RenderSpace::WorldSpace), LAYER_WORLD_MIDGROUND);
	}
}
