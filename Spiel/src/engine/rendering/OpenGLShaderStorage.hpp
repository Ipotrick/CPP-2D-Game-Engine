...
public:
	void initialize(size_t maxSize, OpenGLShader& shader, int binding)
	{
		initialize(maxSize, shader.getNativeHandle(), binding);
	}

	void initialize(size_t maxSize, GLuint shader, int binding)
	{
		assert(shader != -1);
		if (ssbo != -1) reset();
