	...
	inline void executeSample(uint32_t id, CompDataPlayer& data, World& world, float deltaTime) override {
		...
		if (engine.keyPressed(KEY::F)) {
			...
			for (uint64_t i = 0; i < bullets; i++) {
				Entity bullC = Entity(entity->getPos() + rotate(vec2(-entity->getSize().y, 0) / 1.9f, entity->rotation + 270), 0, Collidable(bulletSize, Form::CIRCLE, true, entity->velocity + bulletVel * rotate(vec2(0, 1), entity->rotation + velOffsetRota)));
				CompDataDrawable bullD = CompDataDrawable(vec4(0.f, 1.f, 0.f, 1), bulletSize, 0.4f, Form::CIRCLE);
				world.spawnSolidEntity(bullC ,bullD, CompDataSolidBody(1, 0.001f));
